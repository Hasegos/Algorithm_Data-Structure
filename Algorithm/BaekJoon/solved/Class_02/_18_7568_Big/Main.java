...
/*
    ...
    문제 : 우리는 사람의 덩치를 키와 몸무게, 이 두 개의 값으로 표현하여 그 등수를 매겨보려고 한다.
    ...
    입력받은 키와 몸무게 두개의 값을 모두 비교해야한다.
    그래서 브루트포스 방법으로 배열의 입력값을 모두 비교하는 방식으로 해야한다.
    랭킹이라는 수를 줘서 몇등인지 알아내면된다.
    다만, 랭킹이 높다는것은 등수가 낮다는 의미이기에 한개씩 비교할때 작을 때에만 랭킹 수를 증가 시켜주면된다.
